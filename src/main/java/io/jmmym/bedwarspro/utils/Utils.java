...
public final class Utils {
  ...
  public static void createParticleInGame(Game game, String particle, Location loc) {
    try {
      Class<?> clazz = Class.forName("io.jmmym.bedwarspro.com."
          + BedwarsPRO.getInstance().getCurrentVersion().toLowerCase() + ".ParticleSpawner");
  ...
  public static Material getMaterialByConfig(String key, Material defaultMaterial) {
    try {
      String cfg = BedwarsPRO.getInstance().getStringConfig(key, defaultMaterial.name());
  ...
  public static Location locationDeserialize(Object obj) {
    ...
    try {
      ...
      World world = BedwarsPRO.getInstance().getServer().getWorld(section.get("world").toString());
