...
public class PluginUpdater {
  ...
  public PluginUpdater(Plugin plugin, int id, File file, UpdateType type, UpdateCallback callback,
    ...
    final File updaterConfigFile = new File(updaterFile, "BedwarsPRO.yml");
  ...
  private String getCustomVersion() {
    ...
    try {
      ...
      if (BedwarsPRO.getInstance().isMineshafterPresent()) {
      ...
      if (BedwarsPRO.getInstance().getCurrentVersion().startsWith("v1_7")) {
  ...
  private void runUpdater() {
    // Disabled: return;
    if (false && this.url != null && (this.read() && this.versionCheck())) {
