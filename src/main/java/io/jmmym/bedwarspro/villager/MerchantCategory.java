package io.jmmym.bedwarspro.villager;
...
public class MerchantCategory {
  ...
  private String key = null; // 新增：存储配置键，如 "teamupgrade"

  public MerchantCategory(String key, String name, Material item) {
    this(key, name, item, new ArrayList<VillagerTrade>(), new ArrayList<String>(), 0, "bw.base");
  }

  public MerchantCategory(String key, String name, Material item, ArrayList<VillagerTrade> offers,
  ...
  public static HashMap<Material, MerchantCategory> loadCategories(FileConfiguration cfg) {
    ...
    for (String cat : section.getKeys(false)) {
      ...
      // 传入 cat 作为 key
      mc.put(catItem, new MerchantCategory(cat, catName, catItem, offers, lores, order, permission));
    }
  ...
  public static void openCategorySelection(Player p, Game g) {
    ...
    Inventory inv = Bukkit.createInventory(p, size, BedwarsPRO._l(p, "ingame.shop.name"));
    ...
    snowMeta.setDisplayName(BedwarsPRO._l(p, "ingame.shop.newshop"));
  ...
  private static ItemStack setResourceName(ItemStack item) {
    ...
    for (String key : resourceSection.getKeys(false)) {
      ...
      List<Object> resourceList = (List<Object>) BedwarsPRO.getInstance().getConfig()
