...
public class HolographicDisplaysInteraction implements IHologramInteraction {
  ...
  private Hologram createPlayerStatisticHologram(Player player, Location holoLocation) {
    final Hologram holo = HologramsAPI.createHologram(BedwarsPRO.getInstance(), holoLocation);
  ...
  public void loadHolograms() {
    ...
    File file = new File(BedwarsPRO.getInstance().getDataFolder(), "holodb.yml");
    if (file.exists()) {
  ...
  public void onHologramTouch(final Player player, final Hologram holo) {
    ...
    player.removeMetadata("bw-remove-holo", BedwarsPRO.getInstance());
    BedwarsPRO.getInstance().getServer().getScheduler()
        .runTask(BedwarsPRO.getInstance(), new Runnable() {
          ...
          public void run() {
            ...
            player.sendMessage(
                ChatWriter.pluginMessage(ChatColor.GREEN + BedwarsPRO._l("success.holoremoved")));
          }
  ...
  public void unloadHolograms() {
    if (BedwarsPRO.getInstance().isHologramsEnabled()) {
      Iterator<Hologram> iterator = HologramsAPI.getHolograms(BedwarsPRO.getInstance()).iterator();
      while (iterator.hasNext()) {
  ...
  public void updateHolograms() {
    for (final Player player : BedwarsPRO.getInstance().getServer().getOnlinePlayers()) {
      BedwarsPRO.getInstance().getServer().getScheduler()
          .runTask(BedwarsPRO.getInstance(), new Runnable() {
  ...
  public void updateHolograms(final Player player, long delay) {
    BedwarsPRO.getInstance().getServer().getScheduler()
        .runTaskLater(BedwarsPRO.getInstance(), new Runnable() {
  ...
  private void updatePlayerStatisticHologram(Player player, final Hologram holo) {
    ...
    try {
      nameColor = ChatColor.translateAlternateColorCodes('&',
          BedwarsPRO.getInstance().getStringConfig("holographic-stats.name-color", "&7"));

      valueColor = ChatColor.translateAlternateColorCodes('&',
          BedwarsPRO.getInstance().getStringConfig("holographic-stats.value-color", "&e"));
    } catch (Exception ex) {
      BedwarsPRO.getInstance().getBugsnag().notify(ex);
      // nothing to do
    }

    List<String> lines = BedwarsPRO.getInstance().getPlayerStatisticManager()
        .createStatisticLines(statistic,
            BedwarsPRO.getInstance().getBooleanConfig("holographic-stats.show-prefix", false),
            nameColor,
            valueColor);
