...
public class MagnetShoeListener implements Listener {
  ...
  public void onDamage(EntityDamageByEntityEvent ev) {
    ...
    Game game = BedwarsPRO.getInstance().getGameManager().getGameOfPlayer((Player) ev.getEntity());
    if (game == null) {
      return;
    }
  ...
  private boolean rollKnockbackDice() {
    int target = BedwarsPRO.getInstance().getIntConfig("specials.magnetshoe.probability", 75);
