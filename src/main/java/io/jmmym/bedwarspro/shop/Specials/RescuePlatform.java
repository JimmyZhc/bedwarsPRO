...
public class RescuePlatform extends SpecialItem {
  ...
  public void create(Player player, Game game) {
    ...
    int breakTime = BedwarsPRO.getInstance()
    ...
    boolean canBreak =
        BedwarsPRO.getInstance().getBooleanConfig("specials.rescue-platform.can-break", false);
          ...
          if (waitLeft > 0) {
            player.sendMessage(
                ChatWriter.pluginMessage(
                    BedwarsPRO._l(player, "ingame.specials.rescue-platform.left",
    ...
    if (player.getLocation().getBlock().getRelative(BlockFace.DOWN).getType() != Material.AIR) {
      player.sendMessage(
          ChatWriter.pluginMessage(ChatColor.RED + BedwarsPRO._l(player, "errors.notinair")));
      return;
    }
