...
public class ProtectionWall extends SpecialItem {
  ...
  public void create(Player player, Game game) {
    ...
    int breakTime = BedwarsPRO.getInstance().getIntConfig("specials.protection-wall.break-time", 0);
    int waitTime = BedwarsPRO.getInstance().getIntConfig("specials.protection-wall.wait-time", 20);
    int width = BedwarsPRO.getInstance().getIntConfig("specials.protection-wall.width", 4);
    int height = BedwarsPRO.getInstance().getIntConfig("specials.protection-wall.height", 4);
    int distance = BedwarsPRO.getInstance().getIntConfig("specials.protection-wall.distance", 2);
    boolean canBreak =
        BedwarsPRO.getInstance().getBooleanConfig("specials.protection-wall.can-break", true);
    ...
    if (player.getEyeLocation().getBlock().getType() != Material.AIR) {
      player.sendMessage(
          ChatWriter
              .pluginMessage(
                  BedwarsPRO._l(player, "ingame.specials.protection-wall.not-usable-here")));
      return;
    }
          ...
          if (waitLeft > 0) {
            player.sendMessage(
                ChatWriter.pluginMessage(
                    BedwarsPRO._l(player, "ingame.specials.protection-wall.left",
