...
public class ArrowBlocker extends SpecialItem {
  ...
  public void create(Player player, Game game) {
    ...
    int protectionTime = BedwarsPRO.getInstance()
        .getIntConfig("specials.arrow-blocker.protection-time", 10);
    int waitTime = BedwarsPRO
    ...
    if (waitTime > 0) {
      ...
      if (!livingBlockers.isEmpty()) {
        for (ArrowBlocker livingBlocker : livingBlockers) {
          ...
          if (waitLeft > 0) {
            player.sendMessage(
                ChatWriter.pluginMessage(BedwarsPRO._l(player, "ingame.specials.arrow-blocker.left",
    ...
    player
        .sendMessage(ChatWriter.pluginMessage(
            BedwarsPRO._l(player, "ingame.specials.arrow-blocker.start",
  ...
  public void runTask(final int protectionTime, final int waitTime, final Player player) {
    this.task = new BukkitRunnable() {
      ...
      public void run() {
        ...
        if (protectionTime > 0 && ArrowBlocker.this.livingTime == protectionTime) {
          player.sendMessage(
              ChatWriter.pluginMessage(BedwarsPRO._l(player, "ingame.specials.arrow-blocker.end")));
          isActive = false;
        }
    ...
    }.runTaskTimer(BedwarsPRO.getInstance(), 0, 20L);
