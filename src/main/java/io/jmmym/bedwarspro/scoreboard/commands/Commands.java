package io.jmmym.bedwarspro.scoreboard.commands;
	...
	public boolean onCommand(CommandSender sender, Command cmd, String label, String[] args) {
		if (cmd.getName().equalsIgnoreCase("bedwarsscoreboardaddon")) {
			if (args.length == 0) {
				...
				sender.sendMessage("");
				sender.sendMessage("§f  " + Main.getInstance().getLocaleConfig().getLanguage("author") + ": §aRam" + "       §eModified By JmmYm");
				sender.sendMessage("");
				sender.sendMessage("§f=====================================================");
				return true;
			}
			if (args[0].equalsIgnoreCase("help")) {
				sender.sendMessage("§f=====================================================");
				sender.sendMessage("");
				sender.sendMessage("§b§l BedwarsScoreBoardAddon §fv" + Main.getVersion() + "  §7by Ram       §eModified By JmmYm");
				sender.sendMessage("");
			...
			if (args[0].equalsIgnoreCase("edit")) {
				...
				Game game = BedwarsPRO.getInstance().getGameManager().getGame(args[1]);
				if (game == null) {
			...
			if (args[0].equalsIgnoreCase("time")) {
				if (!sender.hasPermission("bedwarsscoreboardaddon.debug")) {
					sender.sendMessage(Config.getLanguage("commands.message.prefix") + Config.getLanguage("commands.message.no_permission"));
					return true;
				}
				if (args.length < 3) {
					sender.sendMessage("§c用法: /bwsba time <游戏名称> <剩余秒数>");
					sender.sendMessage("§7示例: /bwsba time world 3420  (设置游戏剩余3420秒，触发金锭加速)");
					sender.sendMessage("§7示例: /bwsba time world 3240  (设置游戏剩余3240秒，触发僵尸来袭)");
					sender.sendMessage("§7示例: /bwsba time world 3060  (设置游戏剩余3060秒，触发随机事件)");
					return true;
				}
				Game game = BedwarsPRO.getInstance().getGameManager().getGame(args[1]);
				if (game == null) {
					sender.sendMessage("§c游戏不存在: " + args[1]);
					return true;
				}
				try {
					int time = Integer.parseInt(args[2]);
					game.setTimeLeft(time);
					sender.sendMessage("§a游戏时间已设置为: " + time + " 秒");
