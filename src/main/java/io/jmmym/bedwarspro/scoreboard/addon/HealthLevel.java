...
import io.jmmym.bedwarspro.scoreboard.events.BoardAddonSetHealthEvent;
...
public class HealthLevel {
	...
	public HealthLevel(Arena arena) {
		...
		if (Main.getInstance().getConfig().getConfigurationSection("sethealth") != null) {
						...
						public void run() {
							...
							if (game.getTimeLeft() <= gametime) {
								...
								for (Player player : game.getPlayers()) {
									...
									if (title != null && (!title.equals("") || !subtitle.equals(""))) {
										Utils.sendTitle(player, 10, 50, 10, ColorUtil.color(title), ColorUtil.color(subtitle != null ? subtitle : ""));
									}
