...
import io.jmmym.bedwarspro.scoreboard.addon.SelectTeam;
...
public class GameListener implements Listener {
		...
		new BukkitRunnable() {
			...
			public void run() {
				if (Main.getInstance().getArenaManager().getArenas().containsKey(game.getName())) {
					ScoreBoard sb = Main.getInstance().getArenaManager().getArenas().get(game.getName()).getScoreBoard();
					if (sb != null) {
						sb.updateScoreboard();
					}
				}
	...
	public void onPlayerKilled(BedwarsPlayerKilledEvent e) {
		Game game = BedwarsPRO.getInstance().getGameManager().getGameOfPlayer(e.getKiller());
