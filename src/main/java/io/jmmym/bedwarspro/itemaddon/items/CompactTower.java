...
public class CompactTower implements Listener {
    ...
    private void loadBlocks() {
        blocks = new ArrayList<>();
        try {
            URL url = Main.getInstance().getPlugin().getClass().getResource("/Tower.blocks");
    ...
    public void setblock(Game game, Team team, Location location, Player player) {
        ...
        }.runTaskTimer(BedwarsPRO.getInstance(), 0, 3L);
    }
    ...
    private boolean isCanPlace(Game game, Location location) {
        ...
        for (Entity entity : location.getWorld().getNearbyEntities(location.clone().add(0.5, 1, 0.5), 0.5, 1, 0.5)) {
            ...
            if (io.jmmym.bedwarspro.scoreboard.Main.getInstance() != null && BedwarsUtil.isRespawning(player)) {
                continue;
            }
            return false;
        }
        if (io.jmmym.bedwarspro.scoreboard.Main.getInstance() != null) {
            if (io.jmmym.bedwarspro.scoreboard.config.Config.spawn_no_build_spawn_enabled) {
                for (Team team : game.getTeams().values()) {
                    if (team.getSpawnLocation().distanceSquared(block.getLocation().clone().add(0.5, 0, 0.5)) <= Math.pow(io.jmmym.bedwarspro.scoreboard.config.Config.spawn_no_build_spawn_range, 2)) {
                        return false;
                    }
                }
            }
            if (io.jmmym.bedwarspro.scoreboard.config.Config.spawn_no_build_resource_enabled) {
                for (ResourceSpawner spawner : game.getResourceSpawners()) {
                    if (spawner.getLocation().distanceSquared(block.getLocation().clone().add(0.5, 0, 0.5)) <= Math.pow(io.jmmym.bedwarspro.scoreboard.config.Config.spawn_no_build_resource_range, 2)) {
                        return false;
                    }
                }
                if (io.jmmym.bedwarspro.scoreboard.config.Config.game_team_spawner.containsKey(game.getName())) {
                    for (List<Location> locs : io.jmmym.bedwarspro.scoreboard.config.Config.game_team_spawner.get(game.getName()).values()) {
                        for (Location loc : locs) {
                            if (loc.distanceSquared(block.getLocation().clone().add(0.5, 0, 0.5)) <= Math.pow(io.jmmym.bedwarspro.scoreboard.config.Config.spawn_no_build_resource_range, 2)) {
                                return false;
                            }
                        }
                    }
