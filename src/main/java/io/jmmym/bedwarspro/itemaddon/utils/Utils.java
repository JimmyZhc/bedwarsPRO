...
public class Utils {
    ...
    public static void sendPlayerActionbar(Player player, String text) {
        try {
            Class.forName("io.jmmym.bedwarspro.com." + BedwarsPRO.getInstance().getCurrentVersion().toLowerCase() + ".ActionBar").getDeclaredMethod("sendActionBar", Player.class, String.class).invoke(null, player, text);
