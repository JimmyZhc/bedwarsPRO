...
public class ResourceSpawner implements Runnable, ConfigurationSerializable {
  ...
  public ResourceSpawner(Map<String, Object> deserialize) {
    ...
    if (deserialize.containsKey("name")) {
      ...
      if (BedwarsPRO.getInstance().getConfig().contains("resource." + name)) {
        ...
        this.interval =
            BedwarsPRO.getInstance().getIntConfig("resource." + name + ".spawn-interval", 1000);
        this.spread =
            BedwarsPRO.getInstance().getConfig().getDouble("resource." + name + ".spread", 1.0);
        this.name = name;
      } else {
        List<Object> resourceList = (List<Object>) BedwarsPRO.getInstance().getConfig()
            .getList("resource." + name + ".item");
  ...
  public void run() {
    ...
    for (ItemStack itemStack : this.resources) {
      ...
      BedwarsPRO.getInstance().getServer().getPluginManager().callEvent(resourceSpawnEvent);
      ...
      if (BedwarsPRO.getInstance().getBooleanConfig("spawn-resources-in-chest", true)) {
