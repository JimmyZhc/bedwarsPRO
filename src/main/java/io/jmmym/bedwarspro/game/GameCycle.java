...
import io.jmmym.bedwarspro.shop.Specials.RescuePlatform;
...
public abstract class GameCycle {
  ...
  public void checkGameOver() {
    if (!BedwarsPRO.getInstance().isEnabled()) {
      return;
    }
  ...
  public void onPlayerDies(Player player, Player killer) {
    ...
    BedwarsPRO.getInstance().getServer().getPluginManager().callEvent(killedEvent);
    ...
    if (BedwarsPRO.getInstance().statisticsEnabled()) {
      ...
      if (killer != null) {
        if ((onlyOnBedDestroy && teamIsDead) || !onlyOnBedDestroy) {
          ...
          if (killerPlayer != null) {
            ...
            killerPlayer.setCurrentScore(killerPlayer.getCurrentScore() + BedwarsPRO
      ...
      if (BedwarsPRO.getInstance().getBooleanConfig("rewards.enabled", false) && killer != null
        ...
        BedwarsPRO.getInstance().dispatchRewardCommands(commands,
            ImmutableMap.of("{player}", killer.getName(), "{score}",
                String
                    .valueOf(BedwarsPRO.getInstance().getIntConfig("statistics.scores.kill", 10))));
      }
    }
        ...
        if (aPlayer.isOnline()) {
          aPlayer.sendMessage(
              ChatWriter.pluginMessage(
                  ChatColor.GOLD + BedwarsPRO._l(aPlayer, "ingame.player.died", ImmutableMap
                      .of("player",
    ...
    for (Player aPlayer : this.getGame().getPlayers()) {
      if (aPlayer.isOnline()) {
        aPlayer.sendMessage(
            ChatWriter.pluginMessage(ChatColor.GOLD + BedwarsPRO._l(aPlayer, "ingame.player.killed",
  ...
  public void onPlayerRespawn(PlayerRespawnEvent pre, Player player) {
    ...
    if (team.isDead(this.getGame())) {
      ...
      if (BedwarsPRO.getInstance().spectationEnabled()) {
      ...
      if (BedwarsPRO.getInstance().getRespawnProtectionTime() > 0) {
  ...
  private void runGameOver(Team winner) {
    ...
    int delay = BedwarsPRO.getInstance().getConfig().getInt("gameoverdelay"); // configurable
    // delay

    if (BedwarsPRO.getInstance().statisticsEnabled()
        || BedwarsPRO.getInstance().getBooleanConfig("rewards.enabled", false)
        || (BedwarsPRO.getInstance().getBooleanConfig("titles.win.enabled", true))) {
      if (winner != null) {
        for (Player player : winner.getPlayers()) {
          String title = this
              .winTitleReplace(BedwarsPRO._l(player, "ingame.title.win-title"), winner);
          String subtitle = this
              .winTitleReplace(BedwarsPRO._l(player, "ingame.title.win-subtitle"), winner);
          if (!"".equals(title) || !"".equals(subtitle)) {
            if (BedwarsPRO.getInstance().getBooleanConfig("titles.win.enabled", true)
              ...
              try {
                ...
                if (!"".equals(title)) {
                  double titleFadeIn =
                      BedwarsPRO.getInstance().getConfig()
                          .getDouble("titles.win.title-fade-in", 1.5);
                  double titleStay =
                      BedwarsPRO.getInstance().getConfig().getDouble("titles.win.title-stay", 5.0);
                  double titleFadeOut =
                      BedwarsPRO
          ...
          if (BedwarsPRO.getInstance().getBooleanConfig("rewards.enabled", false)) {
            ...
            commands = (List<String>) BedwarsPRO.getInstance().getConfig()
          ...
          if (BedwarsPRO.getInstance().statisticsEnabled()) {
            ...
            statistic.setCurrentScore(statistic.getCurrentScore() + BedwarsPRO
                .getInstance().getIntConfig("statistics.scores.win", 50));

            if (madeRecord) {
              statistic.setCurrentScore(statistic.getCurrentScore() +
                  BedwarsPRO.getInstance().getIntConfig("statistics.scores.record", 100));
            }
          }
        }
      }
      ...
      for (Player player : this.game.getPlayers()) {
        ...
        if (BedwarsPRO.getInstance().getBooleanConfig("rewards.enabled", false)) {
          List<String> commands = new ArrayList<String>();
          commands =
              (List<String>) BedwarsPRO.getInstance().getConfig()
    ...
    gameOver.runTaskTimer(BedwarsPRO.getInstance(), 0L, 20L);
  }
    ...
    if (deathTeam.getPlayers().size() == 1 && deathTeam.isDead(this.getGame())) {
      for (Player aPlayer : this.getGame().getPlayers()) {
        if (aPlayer.isOnline()) {
          aPlayer.sendMessage(
              ChatWriter.pluginMessage(
                  ChatColor.RED + BedwarsPRO._l(aPlayer, "ingame.team-dead", ImmutableMap.of("team",
    ...
    if (playTime <= this.getGame().getRecord()) {
      ...
      if (!throughBed) {
        for (Player aPlayer : this.getGame().getPlayers()) {
          if (aPlayer.isOnline()) {
            aPlayer.sendMessage(
                ChatWriter.pluginMessage(BedwarsPRO._l(aPlayer, "ingame.record-nobeddestroy")));
          }
        }
        return false;
      }
      ...
      for (Player aPlayer : this.getGame().getPlayers()) {
        if (aPlayer.isOnline()) {
          aPlayer.sendMessage(
              ChatWriter.pluginMessage(BedwarsPRO._l(aPlayer, "ingame.newrecord",
