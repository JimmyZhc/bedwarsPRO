...
public class GameLobbyCountdown extends BukkitRunnable {
  ...
  public GameLobbyCountdown(Game game) {
    this.game = game;
    this.counter = BedwarsPRO.getInstance().getConfig().getInt("lobbytime");
    this.lobbytime = this.counter;
    this.lobbytimeWhenFull = BedwarsPRO.getInstance().getConfig().getInt("lobbytime-full");
  }
  ...
  public void run() {
    ...
    if (!this.game.isStartable()) {
      if (!this.game.hasEnoughPlayers()) {
        ...
        for (Player aPlayer : players) {
          if (aPlayer.isOnline()) {
            aPlayer.sendMessage(ChatWriter.pluginMessage(
                ChatColor.RED + BedwarsPRO._l(aPlayer, "lobby.cancelcountdown.not_enough_teams")));
          }
        }
      }
    ...
    if (this.counter <= 10 && this.counter > 0) {
      ...
      String title = ChatColor.translateAlternateColorCodes('&',
          BedwarsPRO.getInstance().getStringConfig("titles.countdown.format", "&3{countdown}"));
      title = title.replace("{countdown}", String.valueOf(this.counter));

      if (BedwarsPRO.getInstance().getBooleanConfig("titles.countdown.enabled", true)) {
        try {
          titleClass = BedwarsPRO.getInstance().getVersionRelatedClass("Title");
    ...
    if (this.counter == 0) {
      ...
      this.game.start(BedwarsPRO.getInstance().getServer().getConsoleSender());
      return;
    }
