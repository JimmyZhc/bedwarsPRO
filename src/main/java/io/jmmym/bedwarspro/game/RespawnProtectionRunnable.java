...
public class RespawnProtectionRunnable extends BukkitRunnable {
  ...
  public void run() {
    if (this.length > 0) {
      this.player
          .sendMessage(ChatWriter.pluginMessage(BedwarsPRO._l(player, "ingame.protectionleft",
    ...
    if (this.length <= 0) {
      this.player
          .sendMessage(
              ChatWriter.pluginMessage(BedwarsPRO._l(this.player, "ingame.protectionend")));
  ...
  public void runProtection() {
    this.runTaskTimerAsynchronously(BedwarsPRO.getInstance(), 5L, 20L);
  }
