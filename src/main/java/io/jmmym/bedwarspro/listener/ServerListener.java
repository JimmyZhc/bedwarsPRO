...
public class ServerListener extends BaseListener {
  ...
  private String getStatus(Game game) {
    ...
    if (game.getState() == GameState.WAITING && game.isFull()) {
      status = ChatColor.RED + BedwarsPRO._l("sign.gamestate.full");
    } else {
      status = BedwarsPRO._l("sign.gamestate." + game.getState().toString().toLowerCase());
    }
  ...
  public void onServerListPing(ServerListPingEvent slpe) {
    ...
    if (BedwarsPRO.getInstance().getGameManager() == null
        || BedwarsPRO.getInstance().getGameManager().getGames() == null
        || BedwarsPRO.getInstance().getGameManager().getGames().size() == 0) {
      return;
    }

    Game game = BedwarsPRO.getInstance().getGameManager().getGames().get(0);
    ...
    switch (game.getState()) {
      case STOPPED:
        slpe.setMotd(replacePlaceholder(game, ChatColor.translateAlternateColorCodes('&',
            BedwarsPRO.getInstance().getConfig().getString("bungeecord.motds.stopped"))));
        break;
      case WAITING:
        if (game.isFull()) {
          slpe.setMotd(replacePlaceholder(game, ChatColor.translateAlternateColorCodes('&',
              BedwarsPRO.getInstance().getConfig().getString("bungeecord.motds.full"))));
        } else {
          slpe.setMotd(replacePlaceholder(game, ChatColor.translateAlternateColorCodes('&',
              BedwarsPRO.getInstance().getConfig().getString("bungeecord.motds.lobby"))));
        }

        break;
      case RUNNING:
        slpe.setMotd(replacePlaceholder(game, ChatColor.translateAlternateColorCodes('&',
            BedwarsPRO.getInstance().getConfig().getString("bungeecord.motds.running"))));
        break;
      default:
  ...
  private String replacePlaceholder(Game game, String line) {
    ...
    finalLine = finalLine.replace("$title$", BedwarsPRO._l("sign.firstline"));
