...
public class BlockListener extends BaseListener {
  ...
  public void onBlockGrow(BlockGrowEvent grow) {
    ...
    Game game =
        BedwarsPRO.getInstance().getGameManager().getGameByLocation(grow.getBlock().getLocation());
    if (game == null) {
      return;
    }
  ...
  public void onBreak(BlockBreakEvent e) {
    ...
    if (p == null) {
      ...
      Game game = BedwarsPRO.getInstance().getGameManager().getGameByLocation(block.getLocation());
      if (game == null) {
        return;
      }
    ...
    if (!g.getRegion().isPlacedBlock(breakedBlock)) {
      ...
      if (BedwarsPRO.getInstance().isBreakableType(breakedBlock.getType())) {
      ...
      if (!BedwarsPRO.getInstance().getBooleanConfig("friendlybreak", true)) {
        ...
        for (Player player : playerTeam.getPlayers()) {
          ...
          if (player.getLocation().getBlock().getRelative(BlockFace.DOWN).equals(e.getBlock())) {
            p.sendMessage(
                ChatWriter
                    .pluginMessage(ChatColor.RED + BedwarsPRO._l(p, "ingame.no-friendlybreak")));
            e.setCancelled(true);
            return;
          }
        }
      ...
      if (e.getBlock().getType() == Material.ENDER_CHEST) {
        for (Team team : g.getTeams().values()) {
          ...
          if (teamChests.contains(breakedBlock)) {
            ...
            for (Player aPlayer : team.getPlayers()) {
              if (aPlayer.isOnline()) {
                aPlayer.sendMessage(
                    ChatWriter.pluginMessage(BedwarsPRO._l(aPlayer, "ingame.teamchestdestroy")));
              }
            }
            break;
          }
        ...
        meta.setDisplayName(BedwarsPRO._l("ingame.teamchest"));
  ...
  public void onForm(BlockFormEvent form) {
    ...
    Game game =
        BedwarsPRO.getInstance().getGameManager().getGameByLocation(form.getBlock().getLocation());
    if (game == null) {
      return;
    }
  ...
  public void onIgnite(BlockIgniteEvent ignite) {
    ...
    if (ignite.getIgnitingBlock() == null) {
      if (ignite.getIgnitingEntity() instanceof Player) {
        game = BedwarsPRO.getInstance().getGameManager()
  ...
  public void onPlace(BlockPlaceEvent bpe) {
    ...
    if (game.getState() == GameState.RUNNING) {
      ...
      if (replacedBlock != null && !BedwarsPRO
  ...
  public void onSpread(BlockSpreadEvent spread) {
    ...
    Game game =
        BedwarsPRO.getInstance().getGameManager()
  ...
  public void onStructureGrow(StructureGrowEvent grow) {
    ...
    Game game = BedwarsPRO.getInstance().getGameManager().getGameByLocation(grow.getLocation());
    if (game == null) {
      return;
    }
