package io.jmmym.bedwarspro.com.fallback;
...
public class PerformRespawnRunnable extends BukkitRunnable {
  ...
  private Object getPacketObject(String packetName, Class<?>[] constructorClasses,
    ...
    try {
      Class<?> clazz = BedwarsPRO.getInstance().getMinecraftServerClass(packetName);
      if (clazz == null) {
        // TODO
      }
  ...
  public void run() {
    try {
      Class<?> enumClientCommand = BedwarsPRO.getInstance()
          .getMinecraftServerClass("EnumClientCommand");
      Class<?> packetClass =
          BedwarsPRO.getInstance().getMinecraftServerClass("PacketPlayInClientCommand");
      ...
      Class<?> craftPlayerClass = BedwarsPRO.getInstance()
      ...
      Class<?> playerConnectionClass =
          BedwarsPRO.getInstance().getMinecraftServerClass("PlayerConnection");
